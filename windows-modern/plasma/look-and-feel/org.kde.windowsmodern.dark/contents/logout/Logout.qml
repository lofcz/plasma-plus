...
Item {
    ...
    // Instant Restart/Shut Down/Log Out: keep the greeter (required for
    // plasma-shutdown) but do not wait 30s. Leave the all-options screen alone.
    Component.onCompleted: {
        if (!showAllOptions) {
            Qt.callLater(function() { currentAction(); });
        }
    }
